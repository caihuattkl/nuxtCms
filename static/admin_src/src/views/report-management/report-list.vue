<template>
  <div class="app-container">
        ...
        <el-form :inline="true" :model="this.tables.query.form" ref="sForm" size="small" @submit.native.prevent>
          <el-container class="form-container">
            <el-form-item label prop="nickName">
              <el-cascader placeholder="请选择报告分类" clearable v-model="reportClassOptions.newsClassValue" :options="reportClassOptions.selectOption" :props="{ expandTrigger: 'hover' }" @change="_handleChange"></el-cascader>
            </el-form-item>
            <el-form-item label prop="merchantUid">
              <el-input clearable v-model.trim="this.tables.query.form.title" placeholder="报告标题" style="width:300px;"></el-input>
            </el-form-item>
            <el-button-group style="padding-bottom:15px;">
              ...
              <el-button type="primary" :loading="loading" size="small" @click="_queryReportList"><i class="iconfont icon-cf-c13 mini">&nbsp;</i>查询</el-button>
            </el-button-group>
          </el-container>
        </el-form>
      </div>
    ...
    <el-table :data="tables.data" border size="mini">
      ...
      <el-table-column prop="title" width="400" label="报告标题">
        <template slot-scope="scope">
          <a target="_blank" style="color:#606266;text-decoration:underline" :href="`${$g.SITE_DOMAIN}/reports/${scope.row.url}.html`">{{ scope.row.title }}</a>
        </template>
      </el-table-column>
      <el-table-column prop="id" label="所属分类">
        <template slot-scope="scope">
          <span target="_blank">{{ scope.row.firstClassTextZh + " > " + scope.row.childClassTextZh }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="pv" label="点击量"></el-table-column>
      <el-table-column prop="author" label="发布者"></el-table-column>
      <el-table-column prop="keywords" width="300" label="报告关键词"> </el-table-column>
    ...
    <div style="display:flex;justify-content:flex-end;margin-top:10px;">
      <el-pagination small :page-sizes="[10, 20, 50, 100]" @size-change="p => (this.tables.query.pageParams.pageSize = p) && this._queryReportList()" background layout="total, sizes, prev, pager, next, jumper" @current-change="c => (this.tables.query.pageParams.pageNumber = c) && this._queryReportList()" :total="tables.total" />
    </div>
  </div>
</template>
...
import { queryReportClass, queryReportList } from "@/api/reports";
@Component({
...
export default class ReportList extends Vue {
  ...
  /** 获取所有报告分类 */
  async _queryReportClass() {
    let res = await queryReportClass({});
    if (!res.success) return this.$msg.error(res.data.msg);
    this.reportClassOptions.selectOption = res.data.filter((topItem, topIndex) => topItem.first_class == "0");
    this.reportClassOptions.selectOption.forEach(item => {
      item.children = [{ value: "", label: "全部" }];
      item.value = item.directoryName;
      item.label = item.title;
      res.data.forEach((topItem, topIndex) => {
        if (topItem.first_class == item.id) {
          item.children.push({ value: topItem.reports_directory, label: topItem.title });
        }
      });
    });

    console.log(this.reportClassOptions.selectOption);
  }
  // 获取报告内容
  async _queryReportList() {
    let res = await queryReportList({ ...this.tables.query.pageParams });
    if (!res.success) return this.$msg.error(res.data.msg);
